...
class VisitorsLogDetailVC: UIViewController {
...
extension VisitorsLogDetailVC : UIPickerViewDelegate, UIPickerViewDataSource {
...
extension VisitorsLogDetailVC : UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            let materialRejectRemark = vehicleDetail?.exit.materialRejectRemark as? String
            let materialStatus = self.vehicleDetail?.exit.materialStatus
            let materialImageStatus = self.vehicleDetail?.exit.materialImgStatus
            let exitapprovedBy = self.vehicleDetail?.exit.exitApprovedBy
            ...
            if let url = vehicleDetail?.exit.challan as? String, !url.isEmpty {
                cell?.imgOutWardChallan.layer.cornerRadius = 2
                cell?.viewOutwardChallan.isHidden = false
                
                cell?.imgOutWardChallan.sd_setImage(with: URL(string:"\(BaseURL_Img)\(url)"), placeholderImage: UIImage(named: "nopreview"), options: .refreshCached, completed: nil)
            } else {
...
//                cell?.viewMainRemark.isHidden  = true
            ...
            if let materialRemark = vehicleDetail?.exit.materialRejectRemark as? String, !materialRemark.isEmpty {
...
//            if materialStatus == "0" && (materialRejectRemark == nil || materialRejectRemark == "") {
...
//                cell?.viewEntryGrantByName.isHidden = false
            ...
            if vehicleDetail?.exit.exitApprovedBy != "" && vehicleDetail?.exit.exitApprovedBy != "0" {
                cell?.viewExitDetails.isHidden = false
                ...
                if dateTimeArr.count > 1 {
                    ...
                    cell?.lblExitApprovalTime.text = self.vehicleDetail?.exit.approvedDate ?? ""
                }
            }
           
//            let dateStringArr1 = (self.vehicleDetail?.exit.approvedDate ?? "").components(separatedBy: " ")
//
//            if dateStringArr1.count > 1 {
//                let dateIconAtt = getSysImageString("calendar")
//                dateIconAtt.append(NSMutableAttributedString(string: "  \(dateStringArr1[0])  "))
//                dateIconAtt.append(getSysImageString("timer"))
//                dateIconAtt.append(NSMutableAttributedString(string: "  \(dateStringArr1[1])"))
//                cell?.lblExitApprovalTime.attributedText = dateIconAtt
//            } else {
//                cell?.lblExitApprovalTime.text = self.vehicleDetail?.exit.updatedDate ?? ""
//            }
            
            cell?.exitGrantedByView.isHidden = true
            if vehicleDetail?.exit.exitBySecurity != "" && vehicleDetail?.exit.exitBySecurity != "0" {
                cell?.exitGrantedByView.isHidden = false
                cell?.viewExitDetails.isHidden = false
            }

            cell?.lblExitGrantName.text = "\(self.vehicleDetail?.exit.exitBySecurity ?? "") (Gate)"
            let dateTimeArr1 = (self.vehicleDetail?.exit.updatedDate ?? "").components(separatedBy: " ")
...
//                 }
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        ...
        else if indexPath.section == 3 && selectImageFrom != "" {
//            var height = 400.0
//
...
extension VisitorsLogDetailVC : UINavigationControllerDelegate, UIImagePickerControllerDelegate {
...
//MARK:-
extension VisitorsLogDetailVC: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
extension VisitorsLogDetailVC : UITextFieldDelegate {
