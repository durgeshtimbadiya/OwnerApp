...
class VisitorsLogVC: UIViewController , UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        tblFilterRejectedList.isHidden = true
        ...
        tblRejectedList.addSubview(refreshControl)

        if appDelegate.userLoginAccessDetails?.id != nil {
            VisitorLogListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "1", startDate: start_Date, endDate: end_Date, statusType: statuss)
        }
    }
    ...
    @objc func refresh(_: AnyObject) {
        ...
        if appDelegate.userLoginAccessDetails?.id != nil {
            VisitorLogListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "0", startDate: start_Date, endDate: end_Date, statusType: statuss)
        }
    }
    ...
    func showPicker(type _: PickerType) {
        ...
        editRadiusAlert.addAction(UIAlertAction(title: "Done", style: .default, handler: { _ in
            ...
            if self.search_Filter_vehicle_Log_Array.count > 0 {
                ...
                if element == "Today" {
                    self.tblFilterRejectedList.isHidden = true
        ...
        if appDelegate.userLoginAccessDetails?.id != nil {
            if isCustomDateActive == true {
                if txtFieldStartMonth.text == "" {
                    ...
                    FilterApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "1", startDate: start_Date_Value, endDate: end_Date_Value, statusType: statuss)
                }
            } else {
                FilterApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "1", startDate: start_Date_Value, endDate: end_Date_Value, statusType: statuss)
            }
    ...
    @IBAction func btnSearchBarAction(_: Any) {
        ...
        if txtFieldSearch.text != nil && txtFieldSearch.text != "" {
            searchApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", search: txtFieldSearch.text ?? "", search_type: "2", start_date: start_Date_Value, end_date: end_Date_Value, typess: "")
        } else {
    ...
    @IBAction func btnFilterSearchbarAction(_ sender: Any) {
        ...
        if txtFieldFilterSearch.text != nil && txtFieldFilterSearch.text != "" {
            FilterSearchApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", search: txtFieldFilterSearch.text ?? "", search_type: "2", start_date: start_Date_Value, end_date: end_Date_Value, typess: "")
        } else {
    ...
    func VisitorLogListApi(UserID: String,type: String, startDate: String, endDate: String, statusType: String) {
        if ProjectUtilities.checkInternateAvailable(viewController: self) {
            ...
            Webservice.Authentication.visitorAporovalLogApi(parameter: params) { result in
                ...
                case let .success(response):
                            ...
                            if let dictionary = body["request_list"] as? [[String: Any]] {
                                ...
                                if self.all_Entry_Array.count > 0 {
                                    for i in 0 ... self.all_Entry_Array.count - 1 {
                                        if self.all_Entry_Array[i].entry == "1" {
                    ...
                    DispatchQueue.main.async {
                        self.tblRejectedList.delegate = self
                        self.tblRejectedList.dataSource = self
                        self.tblRejectedList.reloadData()
                    }
                    self.tblRejectedList.isHidden = self.entry_Array.count <= 0
                ...
                case let .fail(errorMsg):
                    self.tblRejectedList.isHidden = true
        ...
        if ProjectUtilities.checkInternateAvailable(viewController: self) {
            ...
            Webservice.Authentication.visitorAporovalLogApi(parameter: params) { result in
                ...
                case let .success(response):
                    ...
                    DispatchQueue.main.async {
                        self.tblFilterRejectedList.delegate = self
                        self.tblFilterRejectedList.dataSource = self
                        self.tblFilterRejectedList.reloadData()
                    }
                    self.tblFilterRejectedList.isHidden = self.Filter_entry_Array.count <= 0
    ...
    func searchApproveVehicleListApi(UserID: String,search: String,search_type: String,start_date: String,end_date: String,typess: String) {
        if ProjectUtilities.checkInternateAvailable(viewController: self) {
            let params = ["user_id": UserID,
                          "search": search,
    ...
    func FilterSearchApproveVehicleListApi(UserID: String,search: String,search_type: String,start_date: String,end_date: String,typess: String) {
        if ProjectUtilities.checkInternateAvailable(viewController: self) {
...
extension VisitorsLogVC: UITableViewDelegate, UITableViewDataSource {
    ...
    @objc func btnViewDetailAction(sender: UIButton) {
        DispatchQueue.main.async {
            ...
            if let vc = UIStoryboard(name: "Visitor", bundle: nil).instantiateViewController(withIdentifier: "VisitorsLogDetailVC") as? VisitorsLogDetailVC {
                vc.objApproval = obj
...
extension VisitorsLogVC: UIPickerViewDelegate, UIPickerViewDataSource {
