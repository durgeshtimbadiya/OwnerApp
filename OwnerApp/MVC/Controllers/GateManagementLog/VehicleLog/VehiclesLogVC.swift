...
class VehiclesLogVC: UIViewController , UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        if appDelegate.userLoginAccessDetails?.id != nil {
            VehicleLogListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "0", startDate: start_Date, endDate: end_Date, statusType: statuss)
        }
    }
    ...
    @IBAction func btnSearchBarAction(_: Any) {
        ...
        if txtFieldSearch.text != nil && txtFieldSearch.text != "" {
            searchApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", search: txtFieldSearch.text ?? "", start_date: start_Date_Value, end_date: end_Date_Value)
        } else {
    ...
    @IBAction func btnFilterSearchbarAction(_ sender: Any) {
        ...
        if txtFieldFilterSearch.text != nil && txtFieldFilterSearch.text != "" {
            FilterSearchApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", search: txtFieldFilterSearch.text ?? "", start_date: start_Date_Value, end_date: end_Date_Value)
        } else {
    ...
    func VehicleLogListApi(UserID: String,type: String, startDate: String, endDate: String, statusType: String) {
        if ProjectUtilities.checkInternateAvailable(viewController: self) {
            ...
            Webservice.Authentication.rejectLogApi(parameter: params) { result in
                switch result {
    ...
    func searchApproveVehicleListApi(UserID: String,search: String,start_date: String,end_date: String) {
        if ProjectUtilities.checkInternateAvailable(viewController: self) {
            let params = ["user_id": UserID,
                          "search": search,
    ...
    func FilterSearchApproveVehicleListApi(UserID: String,search: String,start_date: String,end_date: String) {
        if ProjectUtilities.checkInternateAvailable(viewController: self) {
            let params = ["user_id": UserID,
                          "search": search,
                          "start_date": start_date,
...
extension VehiclesLogVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == tblRejectedList {
            ...
            if obj.entry == "1" && obj.exitStatus == "0" {
                ...
                cell.lblStatus.text = "Vehicle Exited"
            }
            ...
            cell.editButton.addTarget(self, action: #selector(btnEditAction(sender:)), for: .touchUpInside)
            return cell
        } else {
            ...
            cell.editButton.tag = indexPath.row
            cell.editButton.addTarget(self, action: #selector(btnEditFilterAction(sender:)), for: .touchUpInside)
...
extension VehiclesLogVC: UIPickerViewDelegate, UIPickerViewDataSource {
