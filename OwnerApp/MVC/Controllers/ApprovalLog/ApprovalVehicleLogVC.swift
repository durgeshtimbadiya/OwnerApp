...
class ApprovalVehicleLogVC: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        if appDelegate.userLoginAccessDetails?.id != nil {
            GetApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "0", startDate: start_Date, endDate: end_Date, statusType: "0")
        }
    }
        ...
        if appDelegate.userLoginAccessDetails?.id != nil {
            if isCustomDateActive == true {
                if txtFieldStartMonth.text == "" {
                    ...
                    FilterApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "0", startDate: start_Date_Value, endDate: end_Date_Value, statusType: "0")
                }
            } else {
                FilterApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "0", startDate: start_Date_Value, endDate: end_Date_Value, statusType: "0")
            }
                    ...
                    if let body = response.body as? [String: Any] {
                        if body["message"] as? String ?? "" == "Success" {
                            ...
                            if let dictionary = body["search_data"] as? [[String: Any]] {
                                ...
                                if self.all_Entry_Array.count > 0 {
                                    for i in 0 ... self.all_Entry_Array.count - 1 {
                                        if self.all_Entry_Array[i].status == "1" && self.all_Entry_Array[i].askApproval == "1"  && self.all_Entry_Array[i].exitStatus == "0" && self.all_Entry_Array[i].exitAskApproval == "0" {
                                            let dic = self.all_Entry_Array[i]
                                            self.entry_Array.append(dic)
                                        } else if self.all_Entry_Array[i].status == "0" && self.all_Entry_Array[i].askApproval == "1"  && self.all_Entry_Array[i].exitStatus == "0" && self.all_Entry_Array[i].exitAskApproval == "0" {
                                            let dic = self.all_Entry_Array[i]
                                            self.entry_Array.append(dic)
                                        }
                                    }
                    ...
                    if let body = response.body as? [String: Any] {
                        if body["message"] as? String ?? "" == "Success" {
                            ...
                            if let dictionary = body["search_data"] as? [[String: Any]] {
                                ...
                                if self.Filter_all_Entry_Array.count > 0 {
                                    for i in 0 ... self.Filter_all_Entry_Array.count - 1 {
                                        if self.Filter_all_Entry_Array[i].status == "1" && self.Filter_all_Entry_Array[i].askApproval == "1"  && self.Filter_all_Entry_Array[i].exitStatus == "0" && self.Filter_all_Entry_Array[i].exitAskApproval == "0" {
                                            let dic = self.Filter_all_Entry_Array[i]
                                            self.Filter_entry_Array.append(dic)
                                        } else if self.Filter_all_Entry_Array[i].status == "0" && self.Filter_all_Entry_Array[i].askApproval == "1"  && self.Filter_all_Entry_Array[i].exitStatus == "0" && self.Filter_all_Entry_Array[i].exitAskApproval == "0" {
                                            let dic = self.Filter_all_Entry_Array[i]
                                            self.Filter_entry_Array.append(dic)
                                        }
                                    }
...
extension ApprovalVehicleLogVC: UITableViewDelegate, UITableViewDataSource {
    ...
    @objc func btnViewDetailAction(sender: UIButton) {
        DispatchQueue.main.async {
            ...
            if let vc = UIStoryboard(name: "Vehicle", bundle: nil).instantiateViewController(withIdentifier: "VehicleDetailsVC") as? VehicleDetailsVC {
                vc.objApproval = obj
...
//                self.navigationController?.pushViewController(vc, animated: true)
    ...
    @objc func btnApproveDetailAction(sender: UIButton) {
        ...
        let alertcontroller = UIAlertController(title: "Confirmation", message: "Are you sure want to Approve this Vehicle?", preferredStyle: .alert)
                        ...
                        case let .success(response):
                            if let body = response.body as? [String: Any] {
                                ...
                                if body["code"] as? Int ?? 0 == 200 {
                                    ...
                                    if appDelegate.userLoginAccessDetails?.id != nil {
                                        self.GetApproveVehicleListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "0", startDate: start_Date, endDate: end_Date, statusType: "0")
                                    }
...
extension ApprovalVehicleLogVC: UIPickerViewDelegate, UIPickerViewDataSource {
