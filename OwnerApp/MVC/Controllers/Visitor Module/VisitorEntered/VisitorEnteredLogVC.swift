...
class VisitorEnteredLogVC: UIViewController ,UITextFieldDelegate{
    ...
    override func viewWillAppear(_: Bool) {
        ...
        if appDelegate.userLoginAccessDetails?.id != nil {
            GetEntryVisitorListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "1", startDate: start_Date, endDate: end_Date, statusType: statuss)
        }
    }
...
//            return
    ...
    func GetEntryVisitorListApi(UserID: String,type: String, startDate: String, endDate: String, statusType: String) {
                        ...
                        if body["message"] as? String ?? "" == "Success" {
                            ...
                            if let dictionary = body["request_list"] as? [[String: Any]] {
                                ...
                                if self.all_Entry_Array.count > 0 {
                                    ...
                                    if self.entry_Array.count > 0 {
                                        self.tblEnteryList.isHidden = false
                            ...
                            if let dictionary = body["request_list"] as? [[String: Any]] {
                                ...
                                if self.Filter_all_Entry_Array.count > 0 {
                                    for i in 0 ... self.Filter_all_Entry_Array.count - 1 {
                                        ...
                                        else if self.Filter_all_Entry_Array[i].entry == "1", self.Filter_all_Entry_Array[i].exitStatus == "0", self.Filter_all_Entry_Array[i].askApproval == "1" {
                                            let dic = self.Filter_all_Entry_Array[i]
                                            self.Filter_entry_Array.append(dic)
                                            self.entry_Array.append(dic)
                                        }
...
extension VisitorEnteredLogVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == tblEnteryList {
            ...
            if obj.exitAskApproval == "0" {
                ...
                cell.btnApproveForExit.isHidden = false
                cell.btnApproveForExit.tag = indexPath.row
                cell.btnApproveForExit.addTarget(self, action: #selector(btnViewFilterApprovalForExitAction(sender:)), for: .touchUpInside)
            } else {
                cell.lblStatus.text = "Seeking For Granted Exit"
                cell.btnExitApproved.isHidden = false
                cell.btnApproveForExit.isHidden = true
                cell.btnExitApproved.tag = indexPath.row
                cell.btnExitApproved.addTarget(self, action: #selector(btnExitFilterApprovedAction(sender:)), for: .touchUpInside)
                }
            
            cell.btnViewDetail.tag = indexPath.row
            cell.btnViewDetail.addTarget(self, action: #selector(btnViewFilterDetailAction(sender:)), for: .touchUpInside)
...
//                self.navigationController?.pushViewController(vc, animated: true)
    ...
    @objc func btnViewFilterDetailAction(sender: UIButton) {
...
//                self.navigationController?.pushViewController(vc, animated: true)
    ...
    @objc func btnViewFilterApprovalForExitAction(sender: UIButton) {
            let obj = self.Filter_entry_Array[sender.tag]
            if let vc = UIStoryboard(name: "Visitor", bundle: nil).instantiateViewController(withIdentifier: "VisitorEnteredLogDetailVC") as? VisitorEnteredLogDetailVC {
                vc.objApproval = obj
                vc.site_Name = self.site_Name
...
//                self.navigationController?.pushViewController(vc, animated: true)
    ...
    @objc func btnExitFilterApprovedAction(sender: UIButton) {
            let obj = self.Filter_entry_Array[sender.tag]
            if let vc = UIStoryboard(name: "Visitor", bundle: nil).instantiateViewController(withIdentifier: "VisitorEnteredLogDetailVC") as? VisitorEnteredLogDetailVC {
                vc.objApproval = obj
                vc.site_Name = self.site_Name
...
extension VisitorEnteredLogVC: UIPickerViewDelegate, UIPickerViewDataSource {
