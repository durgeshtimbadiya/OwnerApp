//
//  VisitorApprovalLogDetailVC.swift
//  EmployeeApp
//
//  Created by Jailove on 26/07/22.
//
...
class VisitorApprovalLogDetailVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        self.GetVisitorRejectApi(UserId: appDelegate.userLoginAccessDetails?.id ?? "", entryId: objApproval?.id ?? "")
    ...
    func GetVisitorRejectApi(UserId: String,entryId: String) {
...
extension VisitorApprovalLogDetailVC : UIPickerViewDelegate, UIPickerViewDataSource {
...
extension VisitorApprovalLogDetailVC : UITableViewDelegate, UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.section == 0 {
            ...
            if let url = visitor_Image, visitor_Image != ""  {
...
//                }
            ...
            if let url = vehicleDetail?.exit.challan as? String, vehicleDetail?.exit.challan as? String != ""  {
            ...
            if let visitorRemark = vehicleDetail?.visitorRemark as? String, vehicleDetail?.visitorRemark != ""  {
...
//                }
            ...
            /*if let MaterialStatus = self.vehicleDetail?.exit.materialStatus , self.vehicleDetail?.exit.materialStatus != "" {
...
//                }
            ...
            if let materialRemark = vehicleDetail?.exit.materialRejectRemark as? String , vehicleDetail?.exit.materialRejectRemark != "" {
//                DispatchQueue.main.async {
//                    self.view.layoutIfNeeded()
                    cell?.viewMaterialRemark.isHidden  = false
//                    self.view.layoutIfNeeded()
//                }
                cell?.txtFieldMaterialRemark.text = materialRemark
            } else {
//                DispatchQueue.main.async {
//                    self.view.layoutIfNeeded()
...
//                }
            ...
            /*if let purposeOfOutPass = self.vehicleDetail?.exit.purposeOfOutPass, self.vehicleDetail?.exit.purposeOfOutPass != "" {
                DispatchQueue.main.async {
                    self.view.layoutIfNeeded()
...
//                }
            ...
            if let url = vehicleDetail?.exit.outpass as? String , vehicleDetail?.exit.outpass != "" {
                cell?.imgOutPass.layer.cornerRadius = 2
                    cell?.viewMainOutPass.isHidden = false

                cell?.imgOutPass.sd_setImage(with: URL(string:"\(BaseURL_Img)\(url)"), placeholderImage: UIImage(named: "nopreview"),
                                             options: .refreshCached,
                                             completed: nil)
            } else {
            ...
            if self.vehicleDetail?.addedBy != "" {
                ...
                if dateTimeArr.count > 1 {
                    ...
                    cell?.lblEntrySentDate.text = self.vehicleDetail?.createdDate ?? ""
                }
            }
            ...
            cell?.lblEntryApprovalName.textColor = .systemRed
            cell?.lblEntryApprovalTime.textColor = .systemRed
            cell?.lblEntryApprovalDate.textColor = .systemRed
...
//                cell?.viewEntryApprovalByName.isHidden = false
                ...
                cell?.lblEntryApprovalName.textColor = UIColor(displayP3Red: 0.0, green: 144.0 / 255.0, blue: 81.0 / 255.0, alpha: 1.0)
                cell?.lblEntryApprovalTime.textColor = UIColor(displayP3Red: 0.0, green: 144.0 / 255.0, blue: 81.0 / 255.0, alpha: 1.0)
                cell?.lblEntryApprovalDate.textColor = UIColor(displayP3Red: 0.0, green: 144.0 / 255.0, blue: 81.0 / 255.0, alpha: 1.0)
            }
...
extension VisitorApprovalLogDetailVC : UINavigationControllerDelegate, UIImagePickerControllerDelegate {
...
//            guard let imageData = selectedImage.jpegData(compressionQuality: 0.4) else { return }
    ...
    func uploadImage(outpass_image: String, meterial_image: String) {
        if ProjectUtilities.checkInternateAvailable(viewController: self) {
            let params = ["user_id":appDelegate.userLoginAccessDetails?.id ?? "","outpass": outpass_image, "material": meterial_image, "site_id": self.site_id] as [String: Any]
            //            print(params)
...
//MARK:-
extension VisitorApprovalLogDetailVC: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
...
extension VisitorApprovalLogDetailVC : UITextFieldDelegate {
