//
//  VisitorRejectedLogVC.swift
//  EmployeeApp
//
//  Created by Jailove on 24/06/22.
//

import UIKit

class VisitorRejectedLogVC: UIViewController, UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        ...
        if appDelegate.userLoginAccessDetails?.id != nil {
            GetRejectVisitorListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "1", startDate: start_Date, endDate: end_Date, statusType: statuss)
        }
    }
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if string.isEmpty && range.location == 0 {
            if txtFieldSearch == textField {
                GetRejectVisitorListApi(UserID: appDelegate.userLoginAccessDetails?.id ?? "", type: "0", startDate: start_Date, endDate: end_Date, statusType: statuss)
    ...
    func GetRejectVisitorListApi(UserID: String,type: String, startDate: String, endDate: String, statusType: String) {
                    ...
                    if let body = response.body as? [String: Any] {
                        if body["message"] as? String ?? "" == "Success" {
                            ...
                            if let dictionary = body["search_data"] as? [[String: Any]] {
                                ...
                                if self.all_Entry_Array.count > 0 {
                                    for i in 0 ... self.all_Entry_Array.count - 1 {
                                        if self.all_Entry_Array[i].status == "2" && self.all_Entry_Array[i].exitStatus == "0" {
                                            let dic = self.all_Entry_Array[i]
                                            self.entry_Array.append(dic)
                                        }
                                    }
                        ...
                        if body["message"] as? String ?? "" == "Success" {
                            ...
                            if let dictionary = body["search_data"] as? [[String: Any]] {
                                ...
                                if self.all_Entry_Array.count > 0 {
                                    ...
                                    for i in 0 ... self.all_Entry_Array.count - 1 {
                                        if self.Filter_all_Entry_Array[i].status == "2" && self.Filter_all_Entry_Array[i].exitStatus == "0" {
                                            let dic = self.Filter_all_Entry_Array[i]
                                            self.Filter_entry_Array.append(dic)
                                        }
                                    }
...
extension VisitorRejectedLogVC: UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == tblRejectedList {
            ...
            let obj = entry_Array[indexPath.row]
                ...
                cell.lblStatus.text = "Visitor Rejected"
    ...
    @objc func btnViewDetailAction(sender: UIButton) {
        DispatchQueue.main.async {
            ...
            if let vc = UIStoryboard(name: "Visitor", bundle: nil).instantiateViewController(withIdentifier: "VisitorRejectedLogDetailVC") as? VisitorRejectedLogDetailVC {
                vc.objApproval = obj
...
extension VisitorRejectedLogVC: UIPickerViewDelegate, UIPickerViewDataSource {
