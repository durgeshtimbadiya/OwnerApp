...
//  EmployeeApp
//
//  Created by Jailove Mewara on 20/12/21.
//

import UIKit

class VehicleLogVC: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        lblTitle.text = "VEHICLE APPROVAL STATUS \n\(site_Name)"
    ...
    func GetVehicleLogApi(user_id: String, start_date: String, end_date: String) {
        if ProjectUtilities.checkInternateAvailable(viewController: self) {
            ...
            Webservice.Authentication.vehicleLogApi(parameter: params) { result in
                switch result {
...
//            navigationController?.pushViewController(vc, animated: true)
    ...
    @IBAction func btnVehicleStatusAction(_: Any) {
        if let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(withIdentifier: "VehicleStatusVC") as? VehicleStatusVC {
            vc.start_Date = start_Date_Value
            vc.end_Date = end_Date_Value
            vc.statuss = ""
            vc.site_Name = site_Name
...
extension VehicleLogVC: UIPickerViewDelegate, UIPickerViewDataSource {
